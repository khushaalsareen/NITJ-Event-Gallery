...
function Events() {
  ...
  useEffect(() => {
    const CodingEvents = Events.filter((ele) => {
      ...
      return ele.eventType === 'Coding' && ele.eventStatus!=='Completed' && (new Date(ele.endDate)>Date.now())
    })
  ...
  useEffect(() => {
    const SportsEvents = Events.filter((ele) => {
      return ele.eventType === 'Sport' && ele.eventStatus!=='Completed' && (new Date(ele.endDate)>Date.now())
    })
    setSportsEvents(SportsEvents)
  }, [Events])
  useEffect(() => {
    const PopularEvents = Events.filter((ele) => {
      return ele.participatedStudents > 0 && (new Date(ele.endDate)>Date.now())
    })
    setPopularEvents(PopularEvents)
  }, [Events])
  useEffect(() => {
    const CulturalEvents = Events.filter((ele) => {
      return  ele.eventType === 'Culture' && ele.eventStatus!=='Completed' && (new Date(ele.endDate)>Date.now())
    })
